...
def setup_authentication(context_obj, zap_step):
    ...
    context_obj[zap_step + "_app_auth"] = {
...
class UploadAction(DropBoxObject):
    ...
    def upload_file(self, trigger_data):
        try:
            ...
            self.dropbox_obj.files_upload(
                trigger_data["upload_data"],
                upload_path,
                mode=files.WriteMode("overwrite"),
            )
            apps_logger.info(
                "Dropbox:UploadAction:ActionSuccessful - ZapID="
    ...
    def check_last_update_time(self):
        try:
            ...
            if file_revision_list.is_deleted:
                ...
                if (
                    current_datetime - latest_revision_datetime
                    ...
                    return (
                        True,
                        "File updated in last "
                        + str(int(update_duration) / 60)
                        + " minutes",
                    )
