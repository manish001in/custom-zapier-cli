...
def trigger_task(zap_id):
    try:
        ...
        if type(output) == str:
            apps_logger.info(
                zap.trigger.app.extra_data["project"] + ":Tasks:TriggerOutput:" + output
            )
        return {"status": status, "trigger_data": output}
    except Exception as e:
        apps_error_logger.info(
            "DropBoxApp:Tasks:TriggerFailureException:ZapID="
            + str(zap_id)
            + ":"
            + str(e)
        )
...
def action_task(trigger_info, zap_id):
    try:
        ...
        apps_error_logger.info(
            "DropBoxApp:Tasks:ActionFailureException:ZapID="
            + str(zap_id)
            + ":"
            + str(e)
        )
