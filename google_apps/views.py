    ...
    def __init__(self, auth_obj, scope):
        ...
        try:
            ...
            apps_error_logger.info(
                "GoogleAuthError:In " + str(self.__class__.__name__) + ", " + str(err)
            )
    ...
    def get_credential_json_object(self):
        ...
        if type(self.auth_obj) == client.OAuth2Credentials:
            ...
            apps_error_logger.info(
                ...
                + ", auth object is not credential type"
            )
...
class GoogleDrive(GoogleObject):
    def __init__(self, auth_obj):
        ...
        self.service = discovery.build(
            "drive", "v3", credentials=self.auth_obj, cache_discovery=False
        )
...
class DownloadFileAction(GoogleDrive):
    ...
    def download_file(self, trigger_data=None):
        try:
            ...
            apps_logger.info(
                "GoogleDrive:DownloadFileAction:ActionSuccessful - ZapID:"
                + str(self.data_obj["zap_id"])
            )

        except HttpError as httperr:
            apps_logger.info(
                "GoogleDrive:DownloadFileAction:ActionException:Got HTTP error, using export_media - ZapID:"
                + str(self.data_obj["zap_id"])
            )
            ...
            apps_error_logger.info(
                "GoogleDriveError:DownloadFileAction:ActionFailed:" + str(err)
            )
...
class FileModifiedTrigger(GoogleDrive):
    ...
    def file_modified(self):
        try:
            file_id = self.data_obj["file_id"]
            update_check_duration_seconds = self.data_obj[
                "check_update_duration_seconds"
            ]
            drive = self.service

            response = (
                drive.files().get(fileId=file_id, fields="modifiedTime").execute()
            )

            modifiedTime = datetime.datetime.strptime(
                response["modifiedTime"], "%Y-%m-%dT%H:%M:%S.%fZ"
            )
            currentUTCTime = core_utility.get_current_utc_datetime()

            if (currentUTCTime - modifiedTime).total_seconds() < float(
                update_check_duration_seconds
            ):
                apps_logger.info(
                    "GoogleDrive:FileModifiedTrigger:Triggered - ZapID:"
                    + str(self.data_obj["zap_id"])
                )
            ...
            apps_error_logger.info(
                "GoogleDriveError:FileModifiedTrigger:TriggerFailed:" + str(err)
            )
...
class GoogleSheets(GoogleObject):
    def __init__(self, auth_obj):
        ...
        self.service = discovery.build(
            "sheets", "v4", credentials=self.auth_obj, cache_discovery=False
        )
...
class AppendRowAction(GoogleSheets):
    ...
    def append_row(self, trigger_data):
        try:
            ...
            body = {"majorDimension": "ROWS", "values": value_list}

            sheet_update_response = (
                sheets.spreadsheets()
                .values()
                .append(
            ...
            apps_logger.info(
                "GoogleDrive:AppendRowAction:ActionSuccessful - ZapID:"
                + str(self.data_obj["zap_id"])
            )

        except Exception as err:
            apps_error_logger.info(
                "GoogleDriveError:AppendRowAction:ActionFailed:" + str(err)
            )
...
class GoogleMail(GoogleObject):
    def __init__(self, auth_obj):
        ...
        self.service = discovery.build(
            "gmail", "v1", credentials=self.auth_obj, cache_discovery=False
        )
...
class EMailTrigger(GoogleMail):
    ...
    def email_trigger(self):
        try:
            after_time = (
                core_utility.get_current_epoch() - 900
            )  # assumes last run time would be 15 mins back and gets that.
            email = self.service
            ...
            if len(return_body) > 0:
                apps_logger.info(
                    "GoogleDrive:EMailTrigger:Triggered - ZapID:"
                    + str(self.data_obj["zap_id"])
                )
            ...
            apps_error_logger.info(
                "GoogleDriveError:EMailTrigger:TriggerFailed:" + str(err), exc_info=True
            )
            return False, return_body
