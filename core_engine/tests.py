...
class UserTestCase(TestCase):
    ...
    def tests_user_create(self):
        self.assertIn("New User Created and UserAuthenticated",new_user(self.username, self.password, None))
        self.assertIn("New User Created and UserAuthenticated",new_user(self.username+'1', self.password+'1', self.email))
    ...
    def tests_user_authenticate(self):
        self.assertIn("New User Created and UserAuthenticated",new_user(self.username, self.password, self.email))

        with self.assertRaises(TypeError):
            authenticate_user()
        with self.assertRaises(TypeError):
            authenticate_user(self.username)
        with self.assertRaisesMessage(PermissionDenied, "User with username "+self.username+"abc"+" does not exist"):
        ...
        self.assertEqual(authenticate_user(self.username, self.password)[0], "UserAuthenticated")
...
class ListingTestCase(TestCase):
    def setUp(self):
        ...
        setup_data = {"text_input": [{"name":"filename", "type":"str"}, {"name":"path", "type":"str"}, {"name":"check_update_duration_seconds", "type":"int"}]}
        ...
        setup_data = {"text_input": [{"name":"filename", "type":"str"}, {"name":"path", "type":"str"}]}
    ...
    def tests_listing(self):
        ...
        self.assertIn("No triggers exist for this app.",list_app_triggers(app_name='GoogleSheetsApp'))
        ...
        self.assertIn("No actions exist for this app.",list_app_actions(app_name='GoogleSheetsApp'))
...
class ZapTestCase(TestCase):
    def setUp(self):
        ...
        setup_data = {"text_input": [{"name":"headers", "type":"str", "options": ["From", "To", "Date", "Subject"], "input_string":"Select the headers you wanna add to your trigger data, if any, in a comma separated way.", "example":"From,To or Subject,To"},
                    {"name":"mailBodyBool", "type":"bool", "not_empty":True, "input_string":"Should we include the mail body text in trigger data?\nPlease answer in yes or no."}, {"name":"search_query", "type":"str", "example": "from:someuser@example.com rfc822msgid:<somemsgid@example.com> is:unread"}]}
        ...
        setup_data = {"text_input": [{"name":"sheet_id", "type":"str", "not_empty":True}, {"name":"sheet_name", "type":"str", "input_string": "Please enter sheet name such as sheet1, sheet2 etc. Do NOT enter the entire spreadsheet name."},
    ...
    def tests_zap(self):
        ...
        with mockRawInput(trigger_fail_answers_1):
            self.assertEqual(False, get_text_input_setup_data(trigger_setup_data['text_input'], True)[0])

        with mockRawInput(trigger_fail_answers_2):
            self.assertEqual(False, get_text_input_setup_data(trigger_setup_data['text_input'], True)[0])

        with mockRawInput(trigger_answers_1):
            status, self.trigger_vals = get_text_input_setup_data(trigger_setup_data['text_input'], True)
        ...
        with mockRawInput(action_answers_1):
            status, self.action_vals = get_text_input_setup_data(action_setup_data['text_input'], True)
            self.assertEqual(True, status)
        ...
        self.assertIn("Successfully marked zap" ,update_zap(zap_name='testzap', mark_status='inactive', user=self.user))

        #No zap with this name exists, so zap query does not exist is expected.
        self.assertIn("query does not exist",update_zap(zap_name='testzap_fail', mark_status='tive', user=self.user))
        
        #No zap with user None exists, so zap query does not exist is expected.
        self.assertIn("query does not exist",update_zap(zap_name='testzap_fail', mark_status='tive', user=None))
